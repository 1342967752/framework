...
var _loadNavigatedAppIds = function () {
	...
	if (save_json_str && save_json_str !== "") {
        ...
        if (typeof save_json["ids"] !== "undefined"
            && G_ServerInfo.getCurServerDayOfYear() === save_json["saveDay"]
            && typeof save_json["ids"] !== "undefined") {
...
export default class AdvLoadMgr extends Laya.Script {
                    ...
                    if (this.isTween) {
                        if (this._advList && this._advList.length > 0) {
                            G_Scheduler.schedule("Auto_Start_Tween_Show_Of_Ad_" + this.advKey, function () {
                                this._autoTweenList()
                            }.bind(this), false, 500, 1)
                        }
                    }

                    if (this._getAdvInfosCb) {
                        let cb = this._getAdvInfosCb
                        this._getAdvInfosCb = null
    ...
    _initUI() {
        if (this.advType === "btn") {
            ...
            if (list) {
                ...
                advList.registerTouchCallback(state => {
                    if (this.isTween) {
                        G_Scheduler.unschedule("Next_Tween_of_Ad_" + this.advKey)

                        if (state === "up") {
                            if (list.scrollBar) {
                                list.scrollBar.once(Laya.Event.END, null, () => {
                                    G_Scheduler.schedule("Next_Tween_of_Ad_" + this.advKey, () => {
                                        this._autoTweenList()
                                    }, false, 1000, 1)
                                })
                            }
                    ...
                    if (advInfo) {
                        this._toMiniProgram(advInfo, function ( bSucc ) {
                            ...
                            if (nextAdvInfo !== null) {
                                ...
                                // report expose
                                if (G_PlatHelper.isWINPlatform()) {
                                    G_Utils.printExposeAdvInfos(nextAdvInfo)
                                }

                                if (G_PlatHelper.getPlat() && G_PlatHelper.getPlat().h_ShowAdvs) {
                                    G_PlatHelper.getPlat().h_ShowAdvs(nextAdvInfo)
                                }
                            }
    ...
    _autoTweenList() {
        if (this.advType === "list" && this.isTween) {
            if (this._advList && this._advList.length > 0) {
                if (this._advList.destroyed) {
                    return
                }
    ...
    hideAdv() {
        console.log("hide adv: ", this.advKey)

        if (this.advType === "list" && this.isTween) {
            G_Scheduler.unschedule("Auto_Start_Tween_Show_Of_Ad_" + this.advKey)
            G_Scheduler.unschedule("Check_Until_All_Advs_Exposed_" + this.advKey)
            G_Scheduler.unschedule("Next_Tween_of_Ad_" + this.advKey)
        }
    }

    randomNavigate( cb ) {
        let doRandomNavigate = () => {
            let randomIndex = G_Utils.random(0, this._advInfos.length - 1)
            let advInfo = this._advInfos[randomIndex]
        ...
        if (!this._advInfos) {
            this.getAdvInfos(doRandomNavigate)
        }
        else {
            doRandomNavigate()
        }
    ...
    getAllExposedAdvInfos() {
        if (this.advType === "list") {
            ...
            if (advList) {
                return advList.getAllExposedAdvInfos()
            }
            else {
                return []
            }
        ...
        if (Array.isArray(this._advInfos)) {
            if (this.advType === "btn") {
                for (let index = 0; index < this._advInfos.length; index++) {
                    ...
                    // report expose
                    if (G_PlatHelper.isWINPlatform()) {
                        G_Utils.printExposeAdvInfos(advInfo)
                    }

                    if (G_PlatHelper.getPlat() && G_PlatHelper.getPlat().h_ShowAdvs) {
                        G_PlatHelper.getPlat().h_ShowAdvs(advInfo)
                    }
                }
            }
            else if (this.advType === "list") {
                ...
                if (advList) {
                    ...
                    if (!this.isTween) {
                        if (G_PlatHelper.isWINPlatform()) {
                            G_Utils.printExposeAdvInfos(cloneAdvInfos)
                        }
    
                        if (G_PlatHelper.getPlat() && G_PlatHelper.getPlat().h_ShowAdvs) {
                            G_PlatHelper.getPlat().h_ShowAdvs(cloneAdvInfos)
                        }
                    }
                    else {
                        G_Scheduler.unschedule("Check_Until_All_Advs_Exposed_" + this.advKey)

                        if (cloneAdvInfos.length > 0) {
                            let exposedAdvIndexs = []
                            for (let index = 0; index < cloneAdvInfos.length; index++) {
                                exposedAdvIndexs.push(false)
                            }
                            ...
                            let doCheck = () => {
                                ...
                                let justExposedAdvInfos = advList.getAllExposedAdvInfos()

                                if (justExposedAdvInfos.length > 0) {
                                    let willExposedAdvInfos = []

                                    for (let i = 0; i < justExposedAdvInfos.length; i++) {
                                        let each = justExposedAdvInfos[i]

                                        if (!exposedAdvIndexs[each.index - 1]) {
                                            exposedAdvIndexs[each.index - 1] = true
                                            willExposedAdvInfos.push(each.info)
                                        }
                                    }

                                    if (willExposedAdvInfos.length > 0) {
                                        // report expose
                                        if (G_PlatHelper.isWINPlatform()) {
                                            G_Utils.printExposeAdvInfos(willExposedAdvInfos)
                                        }
                    
                                        if (G_PlatHelper.getPlat() && G_PlatHelper.getPlat().h_ShowAdvs) {
                                            G_PlatHelper.getPlat().h_ShowAdvs(willExposedAdvInfos)
                                        }

                                        let exposedAdvCount = 0
                                        exposedAdvIndexs.forEach(each => {
                                            if (each) {
                                                exposedAdvCount++
                                            }
                                        })
                                        if (exposedAdvCount === cloneAdvInfos.length) {
                                            G_Scheduler.unschedule("Check_Until_All_Advs_Exposed_" + this.advKey)
                                        }
                                    }
                                }
                            ...
                            G_Scheduler.schedule("Check_Until_All_Advs_Exposed_" + this.advKey, () => {
                                doCheck()
                            }, false, 200)

                            G_Scheduler.schedule("First_Check_Exposed_Advs_" + this.advKey, () => {
                                doCheck()
                            }, false, 10, 0)
                        }
                    }
    ...
    _toMiniProgram( advInfo, touchCb ) {
        ...
        if (advInfo) {
            ...
            let toMin = {
                ...
                origin_path: realAppInfo.logo_url.origin_path,
    ...
    _getWillNavagatedAdvInfo( advInfo ) {
        if (!G_PlatHelper.isWXPlatform() || !G_Switch.isAdvStateNormalSync()) {
            return advInfo
        }
