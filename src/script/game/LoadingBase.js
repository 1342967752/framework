...
export default class LoadingBase extends Laya.Scene {
    ...
    _startLoad() {
        ...
        if (G_BaseUrlPath !== "" && Laya.getMiniAdpter && Laya.getMiniAdpter()) {
            ...
            let miniAdpter = Laya.getMiniAdpter()

            if (miniAdpter) {
                miniAdpter.nativefiles = miniAdpter.nativefiles.concat(appNativefiles)
        ...
        if (G_PlatHelper.getPlat() && !G_PlatHelper.isNativePlatform() && G_GameDB.getBaseConfigByID(BaseConfigIDs["BC_HTTP_ADDR_OF_ASSET_VERSION"]).str !== "") {
            G_AssetMgr.init(() => {
                G_AssetMgr.download(G_PreDownloadAssets, (progress) => {
                    this._onLoadProgress(progress)
                }, () => {
                    // reset progress
                    this._onLoadProgress(0)
