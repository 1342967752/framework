...
export default class GameBase extends Laya.Scene {
    ...
    _onRegisterEvent() {
        ...
        // 本地加载相关
        G_Event.addEventListerner(G_EventName.EN_SHOW_LOCAL_LOADING, function ( content ) {
            if (typeof this.onShowLocalLoading === "function") {
                this.onShowLocalLoading(content)
            }
            else {
        ...
        G_Event.addEventListerner(G_EventName.EN_HIDE_LOCAL_LOADING, function () {
            if (typeof this.onHideLocalLoading === "function") {
                this.onHideLocalLoading()
            }
            else {
