...
else if (typeof window.jsb !== "undefined") {
    plat = window.jsb
}
...
if (plat) {
    ...
    qy_plat.init = function () {
        ...
        var login = function () {
            if (typeof window.qq !== "undefined" || is_wx_plat) {
        ...
        var getNetworkType = function () {
            if (plat.getNetworkType) {
                return new Promise(function (resolve, reject) {
                    plat.getNetworkType({
                        success: function success(res) {
                            g_networkType = res.networkType? res.networkType: ""
                            resolve("")
                        },
                        fail: function fail() {
                            resolve("")
        ...
        var _saveMoreShowAdvInfos = function (advInfos) {
            let allUnsendAdvInfos = null

            let unsendAdvInfos_str = getStorage("qy_unsend_adv_infos")
            if (typeof unsendAdvInfos_str === "string" && unsendAdvInfos_str !== "") {
                allUnsendAdvInfos = JSON.parse(unsendAdvInfos_str)
            }

            if (allUnsendAdvInfos === null) {
                allUnsendAdvInfos = {
                    advs: [],
                    timelog: Date.now()
                }
            ...
            if (Array.isArray(advInfos)) {
                advInfos.forEach(info => {
                    allUnsendAdvInfos.advs.push({
                        adv_id: info.adv_id,
                        appid: info.appid,
                        origin_path: info.origin_path? info.origin_path: info.logo_url.origin_path
                    })
                })
            }
            else if (typeof advInfos === "object") {
                let info = advInfos
            ...
            setStorage("qy_unsend_adv_infos", JSON.stringify(allUnsendAdvInfos))
        }

        var _reportAllSavedAdvInfos = function () {
            let allUnsendAdvInfos = null
            ...
            if (allUnsendAdvInfos !== null) {
                // report
                plat.h_SendEvent("expose", allUnsendAdvInfos)

                // clear
                clearStorage("qy_unsend_adv_infos")
            }
        }
        ...
        var report = function (event_name, event_obj, succCb, failCb) {
            if (!_checkString(event_name) || event_name.length > 255) {
                console.error("上报事件名称必须为String类型且不能超过255个字符")
                return
            }
        ...
        tjconf.app_key = tjconf.app_key.replace(/\s/g, "")
        ...
        if (is_wx_plat) {
            g_domain = 'https://api.game.hnquyou.com/api'
        }
        var g_launchOpts = {}
        var g_code = ""
        var g_networkType = ""
        ...
        // 添加的方法名
        var g_funcNames = ["h_GetAdvListPlat", "h_ShowAdvs", "h_ToMinProgram", "h_SendEvent", "h_JudgeRegion"]
        var g_funcs = {
            ...
            h_ShowAdvs: function (advInfos) {
                _saveMoreShowAdvInfos(advInfos)
            },
            ...
            h_ToMinProgram: function ( _obj ) {
                ...
                var doNavigationReport = function ( type ) {
                    plat.h_SendEvent("clickad", {
                        adv_id: obj.adv_id,
                        origin_path: obj.origin_path,
                        timelog: timelog,
                        type: type
                    })
                }
                ...
                let func = null
                if (plat) {
                    if (plat.navigateToMiniGame) {
                        func = plat.navigateToMiniGame
                    }
                    else if (plat.navigateToMiniProgram) {
                        func = plat.navigateToMiniProgram
                    }
                }
        ...
        // report
        if (plat.h_SendEvent) {
            let _event_name = "qy_sdk_inited"
            if (is_wx_plat) {
                _event_name = "init"
            }

            getNetworkType().then(() => {
                plat.h_SendEvent(_event_name)

                // expose
                _reportAllSavedAdvInfos()
