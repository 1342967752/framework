	...
	function init() {
		...
		var _load = function () {
			if (G_PlatHelper.getPlat() && G_PlatHelper.getPlat().getFileSystemManager) {
				...
				let needLoadFilesCount = G_Dbs.length
				let loadedFilesCount = 0

				for (const index in G_Dbs) {
					let filePath = G_Dbs[index]
					fs.readFile({
						filePath: filePath,
						success: res => {
							let class_name = _getClassName(filePath)
							let serialize_btyes = res.data
							...
							if (loadedFilesCount === needLoadFilesCount) {
								// mark loaded
								_loaded = true;
