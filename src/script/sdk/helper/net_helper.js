		...
		return {
			...
			reqGetWebConfig: function ( cb ) {
				...
				if (G_PlatHelper.isQQPlatform()) {
					...
					sendObj.app_id = G_GameDB.getBaseConfigByID(BaseConfigIDs["BC_HW_MINI_PROGRAM_APP_ID"]).str
				}
				else if (G_PlatHelper.isNativePlatform()) {
					sendObj.app_id = G_GameDB.getBaseConfigByID(BaseConfigIDs["BC_NATIVE_MINI_PROGRAM_APP_ID"]).str
				}
				else {
					sendObj.app_id = G_GameDB.getBaseConfigByID(BaseConfigIDs["BC_MINI_PROGRAM_APP_ID"]).str
				}
			...
			reqWithdrawMoney: function ( sessID, money, advTimesOfToday, totalAdvTimes, cb ) {
				...
				sendObj.opts = JSON.stringify(optionObj)
				sendObj.sign = hex_md5(sessID + hex_md5("money={0}&key=withdrawMoney&money={1}&opts={2}".format(money.toString(), money.toString(), sendObj.opts)))
			...
			reqWithdrawEverydayMoney: function ( sessID, money, advTimesOfToday, totalAdvTimes, recordDayOfEverydayMoney, cb ) {
				...
				sendObj.opts = JSON.stringify(optionObj)
				sendObj.sign = hex_md5(sessID + hex_md5("money={0}&key=withdrawEverydayMoney&money={1}&opts={2}".format(money.toString(), money.toString(), sendObj.opts)))
			...
			_doRequest: function (obj) {
				...
				if (obj) {
					...
					if (G_PlatHelper.isQQPlatform()) {
						...
						header.appId = G_GameDB.getBaseConfigByID(BaseConfigIDs["BC_HW_MINI_PROGRAM_APP_ID"]).str
					}
					else if (G_PlatHelper.isNativePlatform()) {
						header.appId = G_GameDB.getBaseConfigByID(BaseConfigIDs["BC_NATIVE_MINI_PROGRAM_APP_ID"]).str
					}
					else {
