...
var _UIHelper = (function () {
	...
	function init() {
		...
		return {
            ...
            playBtnTouchAction: function (btn, cb, originalScale = 1, scaleExternal = 1, durationScale = 1, clickInterval = 500) {
				// body...
				if (btn) {
			...
			refreshFreeWayOfBtn: function (btn, videoIconPath = "comm/video_icon.png", shareIconPath = "comm/share_icon.png") {
				if (btn) {
					...
					if (!btn.doTouch) {
						btn.doTouch = function ( shareScene, succCb, failCb, showFailTips = true, showTwiceModal = true ) {
								...
								if (btn.getWay() === G_FreeGetWay.FGW_ADV) {
									...
									let checkVideoRet = function ( isEnded, rewatchCb ) {
										...
										if (isEnded) {
											...
											if (!showTwiceModal || G_PlatHelper.isVIVOPlatform()) {
												doFailCb()
											}
											else {
												// no finish
									...
									if (G_PlatHelper.isOVPlatform() || G_PlatHelper.isMZPlatform() || G_PlatHelper.isHWPlatform()) {
										...
										if (G_NativeAdvMgr.isRewardVideoLoaded()) {
											G_NativeAdvMgr.showRewardVideo((isEnded) => {
												checkVideoRet(isEnded, showNativeAdFail)
											}, showNativeAdFail)
										}
										else {
											showNativeAdFail()
			...
			convertToOpenGLSize: function ( worldSize ) {
				// body...
				let openGLSize = new Laya.Size(0, 0)
				...
				openGLSize._width = worldSize.width / Laya.stage.width * sysInfo.screenWidth
				openGLSize._height = worldSize.height / Laya.stage.height * sysInfo.screenHeight
			...
			convertToWorldSize: function ( openGLSize ) {
				...
				worldSize._width = openGLSize.width / sysInfo.screenWidth * Laya.stage.width
				worldSize._width = openGLSize.height / sysInfo.screenHeight * Laya.stage.height
