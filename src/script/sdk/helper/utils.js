var _Utils = (function () {
	...
	function init() {
		...
		return {
			 ...
			 * 
			 */
			convertSecondToHourMinuteSecond: function ( seconds, isAutoHideHour ) {
				// body...
				let ret = ""
			...
			printExposeAdvInfos(advInfos) {
				let doExpose = (advInfo) => {
					console.log("曝光广告：", {
						advId: advInfo.adv_id,
						appId: advInfo.appid,
						title: advInfo.title,
						path: advInfo.origin_path? advInfo.origin_path: advInfo.logo_url.origin_path
					})
				}

				if (Array.isArray(advInfos)) {
					console.warn("曝光广告" + advInfos.length + "条")
