...
export default class PlayerCtrol extends PlayerActor {
    constructor() {
        ...
        this.honeyAnim = null;
        this.honeyTempName="ScaleTemp";

        this.isShowFullTips=false;
        this.isShowCollectTips=false;
        this.jiantou=null;
        this.rotTemp=new Laya.Vector3();
        this.isPlayHurtSound=false;
    }


    init() {
        super.init();
        this.honeyAnim = Tools.getIns().findSprite3D(this.owner, "honey").getComponent(Laya.Animator);
        this.jiantou=this.owner.getChildByName("jiantou");
        if (this.honeyAnim instanceof Laya.Animator) {
            let state= this.honeyAnim.getDefaultState();

            //增加动画
            let scaleTemp = new Laya.AnimatorState();
            scaleTemp.name =this.honeyTempName;
            scaleTemp.clipStart = state.clipStart;
            scaleTemp.clipEnd = state.clipEnd;;
            scaleTemp.clip=state.clip;
            scaleTemp.clip.islooping=false;
            this.honeyAnim.addState(scaleTemp);
    ...
    jianTouLookAt(pos){
        ...
        this.jiantou.transform.getUp(this.tempV2);
        this.rotTemp.y = this.jiantou.transform.localRotationEulerY + 180;
        Laya.Vector3.subtract(pos, this.jiantou.transform.position, this.tempV1);
        Laya.Vector3.subtract(this.jiantou.transform.position, this.tempV1, this.tempV1);
        this.jiantou.transform.lookAt(this.tempV1, this.tempV2, false);
    }
    ...
    setCorl() {
        MapMgr.getIns().gameView.dragCrol.mouseDownFun = () => {
            ...
            if(!this.isShowCollectTips){
                this.isShowCollectTips=true;
                MapMgr.getIns().gameView.showTeachTips(1);
            }
        }
    ...
    toKang() {
        ...
        if(!this.isPlayHurtSound){
            this.isPlayHurtSound=true;
            G_SoundMgr.playSound(GG_SoundName.SN_Mp3.format("hurt"));
        }
    ...
    addHoney(val) {
        ...
        let lastVal=this.curCapcityContain / this.capcityContain;
        this.curCapcityContain += val;
        if (this.curCapcityContain >= this.capcityContain) {
            this.curCapcityContain = this.capcityContain;
            if(!this.isShowFullTips){
                this.isShowFullTips=true;
                MapMgr.getIns().gameView.showTeachTips(2);
            }
        }

        let pro = this.curCapcityContain / this.capcityContain;
        this.setBottle(lastVal,pro);
    ...
    setBottle(last,cur,speed=1){
        let scaleTemp=this.honeyAnim.getControllerLayer(0).getAnimatorState(this.honeyTempName);
        scaleTemp.clipStart=last;
        scaleTemp.clipEnd=cur;
        this.honeyAnim.speed=speed;
        this.honeyAnim.play(this.honeyTempName);
    }
