...
import FucStone from "../Ob/FucStone";
import BeeBox from "../Ob/BeeBox";
export default class MapPart extends Laya.Script {
    ...
    init(key) {
        ...
        let fucStones = [];
        Tools.getIns().getChildBySginInFirstChild(this.owner, "wanted_", fucStones);
        for (let i = 0; i < fucStones.length; i++) {
            let mgr=fucStones[i].addComponent(FucStone);
            let type = parseInt(fucStones[i].name.split('_')[1]);
            mgr.init();
            mgr.setStoneType(type);
            this.fucStones.push(mgr);
            mgr.checkRegist();
        }


        let beeBoxs=[];
        Tools.getIns().getChildBySginInFirstChild(this.owner, "bee_box_", beeBoxs);
        for (let i = 0; i < beeBoxs.length; i++) {
            let mgr=beeBoxs[i].addComponent(BeeBox);
            mgr.init();
            this.beeBoxs.push(mgr);
            mgr.checkRegist();
        }
    }
    ...
    getPlantsByType(type){
        for(let i=0;i<this.plants.length;i++){
            if(this.plants[i].plantType==type){
    ...
    getStoreType(type){
        for(let i=0;i<this.stores.length;i++){
            if(this.stores[i].storeType==type){
    ...
    checkColloder(playerMgr) {
        ...
        this.checkArrayDisPlayer(this.stores,playerMgr);
        //检查功能开启
        if(this.fucStones.length>0){
            this.checkArrayDisPlayer(this.fucStones,playerMgr);
        }

        if(this.beeBoxs.length>=0){
            this.checkArrayDisPlayer(this.beeBoxs,playerMgr);
        }
    ...
    checkArrayDisPlayer(array,playerMgr){
        let temp;
